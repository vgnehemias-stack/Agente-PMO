...
ESTILOS_EXPORT = """
  ...
  /* ===== Impresion y PDF =====
     Un contenedor con desplazamiento no se desplaza al imprimir: se RECORTA.
     Por eso aqui se le devuelve el desbordamiento visible a las tablas anchas
     y a los bloques de codigo, o perderian sus ultimas columnas sin avisar. */
  @media print {
    :root { --barra: 0px; }
    ...
    /* recuperar lo que se recortaria */
    .tablewrap, pre, .listcol { overflow: visible; }
    table { min-width: 0; }
    .listcol { max-height: none; }

    /* nada que quede a caballo entre dos paginas */
    .app, .frag, figure, .nivel, .din, .ck, .nd, .fase, .sub,
    .stat, .block, .gl, .aviso, .paso, .unit, .cap { break-inside: avoid; }
    .cap, .sec-head { break-after: avoid; }

    /* Flujo continuo: el contenido corre y las paginas cortan donde caen.
       Nada de un apartado por hoja, que lee como diapositivas impresas. */
    section { padding-block: 26px 0; }
    .sec-head { break-after: avoid; }
    /* Los 6 apartados en una sola fila: cuadra la rejilla y no deja hueco */
    .toc { grid-template-columns: repeat(6, minmax(0, 1fr)); }

    /* el PDF se pide en claro: se fuerza aqui ademas de en el guion */
